...
- **Setup Wizard & In-Panel Settings**:
  ...
  - Press <kbd>,</kbd> or the `󰒓` button for settings, grouped into **General**, **Security** and **SSH Agent**, each of which folds: auto-lock timeout, clipboard clear delay, TOTP auto-copy delay, and every toggle. The section you are in is named above the list and folds from there, so a long section can be shut without scrolling back to its heading. A setting whose dependency is missing is shown but inert, with the reason given.
