...
Every signature asks first. The prompt says what is being signed -- a Git commit or tag signature, a signature in another `ssh-keygen -Y` namespace, an SSH login as a named user, or data the helper does not recognise -- then names the key, its `SHA256:` fingerprint, and the program asking -- its name and the absolute path it ran from -- and gives you two minutes to answer. Not its pid: the number is gone by the time you could look it up, and the grant is not scoped to it.
