...
- **The settings screen keeps its section heading and its way out in view.** Scrolling twenty rows into Security no longer means scrolling back to the top to fold it: once a section's heading scrolls off, it reappears pinned above the scroll area and folds that section from where you are. A heading still on screen is left alone -- it can be clicked where it is -- so nothing is drawn twice, and the bar clears once you scroll past the last section into the maintenance rows. **Back (Esc)** is pinned alongside it, on the right.
