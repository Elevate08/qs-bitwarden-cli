...
- **The settings screen folds.** It had grown into one scroll of fourteen settings, the SSH agent status block and a row of action buttons. Each group heading is now a row that collapses the settings under it, showing how many it is holding when folded. Behavior and Suggestions, one and two rows each, are merged into a single **General** section that leads the screen instead of being stranded below the SSH agent's block; the groups are now General, Security and SSH Agent. Security starts open because it is what the screen is usually opened for; the rest open on a click, `Enter`, or the arrow keys. The state lasts as long as the session and is not written to `shell.json`.
- **The settings screen keeps its section heading and its way out in view.** Scrolling twenty rows into Security no longer means scrolling back to the top to fold it: the section you are currently in is named above the scroll area from the moment the screen opens, and folds from there wherever you have scrolled to. Its own heading in the list yields to it, so nothing is drawn twice, and the bar clears once you scroll past the last section into the maintenance rows. **Back (Esc)** is pinned alongside it, on the right.
